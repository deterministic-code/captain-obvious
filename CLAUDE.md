...
- **`core/`** — the engine package `@deterministic-code/captain-obvious`. `core/src/` is strict
  ...
  - `core/lib/*.mjs` — install-time helpers. `core/bin/install.mjs` / `core/bin/lint.mjs` — the
    ...
    data + the managed `.gitignore` block); `core/bin/gitignore.mjs` — the `captain-obvious-gitignore`
    bin (adds `.captain-obvious/` to `.gitignore`; install also does this in local mode). These delegate to `core/lib/list-hooks.mjs`/
