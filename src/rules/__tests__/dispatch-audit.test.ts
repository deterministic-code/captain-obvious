...
import { hookScriptPath } from "../dispatch.js";
 ...
 * local git stage must therefore ship `hooks/git/<slug>.mjs`.
 */
describe("dispatch audit — every local-stage rule is runnable", () => {
  ...
  it.each(local.map((r) => r.meta.slug))(
    "%s has a hook implementation on disk",
    (slug) => {
      expect(existsSync(hookScriptPath(slug))).toBe(true);
    },
  );
