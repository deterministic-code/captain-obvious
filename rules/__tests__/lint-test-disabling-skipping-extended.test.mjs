...
describe("extended suppression markers (absolute scan)", () => {
  test("flags fdescribe / fit focus aliases", () => {
    expect(findViolations("fdescribe('s', () => {});\n", "unit").map((v) => v.kind)).toContain(
      "focus-alias",
    );
    expect(findViolations("fit('t', () => {});\n", "unit").map((v) => v.kind)).toContain(
      "focus-alias",
    );
  });

  test("flags .todo", () => {
    expect(findViolations("it.todo('later');\n", "unit").map((v) => v.kind)).toContain("todo");
  });
  ...
  test("does not flag legitimate identifiers (benefit, todos)", () => {
    const kinds = findViolations("const benefit = 1; const todos = [];\n", "unit").map(
      (v) => v.kind,
    );
...
describe("extended markers count as ratchet regressions", () => {
  test("adding fit / .todo / this.skip to a file is a regression", () => {
    ...
    expect(ids).toEqual(expect.arrayContaining(["focus-alias", "todo", "this-skip"]));
  });
});
